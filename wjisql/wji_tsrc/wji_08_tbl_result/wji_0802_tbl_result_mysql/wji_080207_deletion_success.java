/**
 * Tests successful deletion of data.
 */
...
class wji_080207_deletion_success {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        // 6. 2nd row Question number before deleting
        waitForWe = (new WebDriverWait(driver, 10))
                .until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[@id=\"tbl-rs-0\"]/tbody/tr[2]/td[7]")));
        col = driver.findElement(By.xpath("//*[@id=\"tbl-rs-0\"]/tbody/tr[2]/td[7]")); 
        System.out.println("qtn_number before deleting: " + col.getText());  
        
        // 7. Select row 2 for deleting
        button = driver.findElement(By.xpath("//input[@name='vr2c0']"));
        button.click();
        
        // 8. Click buttion 'Delete'
        button = driver.findElement(By.xpath("//input[@value='Delete']"));
        button.click();
        // Wait for the html to appear once again
        waitForWe = (new WebDriverWait(driver, 10))
                .until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[@id='tbl-rs-0']")));
        ...
        // 10. Check deletion of 2nd row containign qtn_number = 2.
        Thread.sleep(1000);
        driver.switchTo().defaultContent();
        driver.switchTo().frame("rightdatafr");
        ...
        for (int i = 1; i <= rows.size(); ++i) {
            ...
            for (int j = 3; j <= cols.size(); ++j) {
                ...
                if (j == 7 ) {
                    System.out.println("Question number: " + col.getText());
                    if (col.getText().equals("2")) {
                        System.out.println("Error: Row 2 was not deleted; question number with vaue 2 still exists.");
                    } 
                }
            }
            System.out.println();
