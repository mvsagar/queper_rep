/**
 * Tests procedure/functions properties screen button 'Drop' for functions.
 */
...
public class wji_050313_func_drop {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        // 5. Display currenr function count
        driver.switchTo().defaultContent();
        driver.switchTo().frame("leftdatafr");

        he = "//*[@id='tbl-funcs']/tbody/tr/td[1]";
        rows = driver.findElements(By.xpath(he));
        System.out.println("No of html tbl rows: " + rows.size());
        System.out.println("Number of functions before dropping: " + rows.size());
        
        
        
        // 6. Display properties of function func_1_1_compute_tax 
        driver.switchTo().defaultContent();
        driver.switchTo().frame("leftdatafr");
        // The following does not work because functions are also shown 
        // in the procedure list. Hence, we need to search only the function list
        // table.
        // we = driver.findElement(By.linkText("func_1_1_compute_tax"));
        for (int i = 1; i <= rows.size(); ++i) {
            he = "//*[@id='tbl-funcs']/tbody/tr[" + i + "]/td[2]";
            we = driver.findElement(By.xpath(he));
            if (we.getText().equalsIgnoreCase("func_1_1_compute_tax")) {
                System.out.println("Found the function 'func_1_1_compute_tax' to be dropped");
                break;
            }
        }
        we.click();
        ...
        // 10. Make sure function count is reduced by 1.
        driver.switchTo().defaultContent();
        driver.switchTo().frame("leftdatafr");
        ...
        rows = driver.findElements(By.xpath(he));
        System.out.println("No of html tbl rows: " + rows.size());
        System.out.println("Number of functions after dropping: " + rows.size());
